  ...
  def diff(list : List[Int], min : Option[Int], max : Option[Int]) : Int = {
    if(list.length == 0) {
      ...
      val newMin = min match {
        case Some(minVal) => if (num < minVal) Some(num) else Some(minVal)
        case None => Some(num)
      }

      val newMax = max match {
        case Some(maxVal) => if (num > maxVal) Some(num) else Some(maxVal)
        case None => Some(num)
      }

      diff(list.tail, newMin, newMax)
    }
  }
}